<?php
    ...
    if ($period_mode === "select_date") {
       ...
       appointment_time,appointment_department,appointment_detail,appointment_status,review_no from appointment a left join hospital h on a.hospital_id=h.id where appointment_date between date(DATE_FORMAT('{$date1}','%Y%m%d')) and date(DATE_FORMAT('{$date2}','%Y%m%d')) order by `date` desc;";

//            $query = "select * from appointment where appointment_date between date(DATE_FORMAT('{$date1}','%Y%m%d')) and date(DATE_FORMAT('{$date2}','%Y%m%d'));";
