<?php
    ...
    if($database_flag === false){
        $sql = "create database todagtodag";
        $value = mysqli_query($con, $sql) or die("Error".mysqli_error($con));
        if($value === true){
            echo "
            <script>
            alert('todagtodag DB가 생성되었습니다.');
            </script>
            ";
        }
    } 
    ...
    $dbcon = mysqli_select_db($con, "todagtodag") or die("Error".mysqli_error($con));
    if(!$dbcon){
        echo "
        <script>
        alert('todagtodag DB가 생성되었습니다.');
